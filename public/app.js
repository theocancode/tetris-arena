...
function updateAuthUI() {
  var bar=$('logged-in-bar'), note=$('not-logged-note'), nm=$('logged-in-name');
  if (currentUser) {
    if(bar)  bar.classList.remove('hidden');
    if(note) note.classList.add('hidden');
    if(nm)   nm.textContent = '👤 ' + currentUser;
    var inp=$('inp-name'); if(inp && !inp.value) inp.value = currentUser;
  } else {
...
function loadMyStats() {
  var grid=$('stats-grid'), title=$('stats-title');
  if (!currentUser) {
  ...
  fetch('/stats/'+currentUser).then(function(r){return r.json();}).then(function(d){
    if (!d.ok || !grid) return;
    var s=d.stats, winRate=s.games_played>0?Math.round(s.wins/s.games_played*100):0;
    var items=[
...
function loadLeaderboard() {
  var list=$('leaderboard-list'); if(!list) return;
...
function rebuildPlayerList() {
  var ul=$('player-list'); if(!ul) return;
  ul.innerHTML='';
  for (var i=0;i<players.length;i++) {
    ...
    var badge=document.createElement('span'); badge.className='player-badge';
    ...
    row.append(dot,name,badge); ul.appendChild(row);
  }
  var startBtn=$('btn-start'), waitMsg=$('waiting-msg');
...
function showRoundSummary(summary, roundWins) {
  var box=$('round-summary'), rows=$('summary-rows');
  if(!box||!rows||!summary) return;
  var entries=Object.values(summary).sort(function(a,b){return (b.wins||0)-(a.wins||0);});
  rows.innerHTML=entries.map(function(p,i){
    return '<div class="summary-row">'+
      '<span class="summary-crown">'+(i===0?'👑':'  ')+'</span>'+
...
window.addEventListener('game-over', function(e){
  var d=e.detail, iWon=d.winnerId===d.myId;
...
document.querySelectorAll('.tab-btn').forEach(function(btn){
  btn.addEventListener('click',function(){
    ...
    var tc=$('tab-'+btn.dataset.tab); if(tc) tc.classList.add('active');
  });
});

// ── All button handlers ───────────────────────────────────────────
$('open-auth-link').addEventListener('click', function(){
  $('auth-modal').classList.remove('hidden');
  var ni=$('inp-name'),ci=$('claim-username');
  if(ni&&ci&&ni.value) ci.value=ni.value;
});
$('btn-close-auth').addEventListener('click', function(){ $('auth-modal').classList.add('hidden'); });
$('btn-do-claim').addEventListener('click', claimProfile);
$('claim-username').addEventListener('keydown', function(e){ if(e.key==='Enter') claimProfile(); });

$('btn-logout').addEventListener('click', function(){
  fetch('/auth/logout',{method:'POST'}).then(function(){ currentUser=null; updateAuthUI(); toast('Logged out'); });
});
...
$('btn-rematch').addEventListener('click', function(){ MusicSystem.play('lobby'); socket.emit('return-to-lobby'); });
$('btn-back-lobby').addEventListener('click', function(){ MusicSystem.play('lobby'); socket.emit('return-to-lobby'); });
$('btn-quit').addEventListener('click', function(){
  MusicSystem.stop(); socket.disconnect(); socket.connect(); players=[];
  if(gm){gm.stop();gm=null;} hideOverlay(); showScreen('landing');
});
$('lobby-code').addEventListener('click', function(){
  navigator.clipboard.writeText(location.origin+'?join='+roomCode).then(function(){ toast('Invite link copied! 📋'); });
});
$('copy-hint').addEventListener('click', function(){ var lc=$('lobby-code'); if(lc) lc.click(); });
