...
class GameManager {
  ...
  start() {
    ...
    this.engine._lock = () => {
      ...
      if (cleared > 0) {
        ...
        setTimeout(() => SoundSystem.playClearVoice(cleared), 120);
  ...
  // stop() — pauses game loop but KEEPS game-over listener alive
  stop() {
    ...
    // intentionally keep game-over listener — dead players still need to receive it
  }

  // stopFull() — called only after game-over is handled
  stopFull() {
    this.stop();
  ...
  _layout() {
    ...
    return {
      boardX: ox + leftW + 8, boardY: oy,
      holdX: ox, holdY: oy + 16,
      statsX: ox, statsY: oy + 16 + R.PANEL * 3 + 24,
      nextX: ox + leftW + 8 + boardW + 14, nextY: oy + 16,
      oppX:  ox + leftW + 8 + boardW + 14 + R.PANEL * 4 + 16, oppY: oy,
      W, H
    };
  }
  ...
  _loop(ts) {
    ...
    if (eng.piece && !eng._fits(eng.piece.shape, eng.piece.row + 1, eng.piece.col)) {
      ...
      if (moved !== this._lastLockMoves) {
        ...
        if (moved < this._LOCK_MAX) {
          this._lockTimer = setTimeout(() => { if (this.engine && !this.engine.isDead) this.engine._lock(); }, this._LOCK_MS);
        } else { this.engine._lock(); }
    ...
    if (ts - this._lastSync > 100) {
      ...
      this.socket.emit('lines-cleared', { total: eng.lines });
    }
  ...
  _onEliminated({ id }) {
    if (this.opponents[id]) this.opponents[id].alive = false;
    SoundSystem.play('ko');
    this._eliminated++;
  ...
  _onDead() {
    this.stop(); // keeps game-over listener alive
    SoundSystem.play('lose');
    setTimeout(() => SoundSystem.play('voice_lose'), 300);
    // Only dispatch local-dead — app.js handles emitting player-dead to server
    window.dispatchEvent(new CustomEvent('local-dead'));
  }

  _onGameOver({ winnerId, winnerName }) {
    this.stopFull(); // now safe to remove game-over listener
    ...
    window.dispatchEvent(new CustomEvent('game-over', {
      detail: { winnerId, winnerName, myId: this.myId }
    }));
  }
}
